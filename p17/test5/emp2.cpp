...
void abstr_emp::writeAll(std::ofstream & fout) {
     cout << "abstr_emp writeAll()\n";
    fout.write(fname.c_str(), fname.size());
    fout.write("\n", 1);
    fout.write(lname.c_str(), lname.size());
    fout.write("\n", 1);
    fout.write(job.c_str(), job.size());
    fout.write("\n",1);
}
...
void manager::writeSelf(std::ofstream & fout) {
    fout.write((char *)&inchargeof, sizeof(int));
    fout.write("\n", 1);
}
...
void fink::writeSelf(std::ofstream & fout) {
    fout.write(reportsto.c_str(), reportsto.size());
    fout.write("\n", 1);
}
...
void highfink::writeSelf(std::ofstream & fout) {
    manager::writeSelf(fout);
    fink::writeSelf(fout);
}
